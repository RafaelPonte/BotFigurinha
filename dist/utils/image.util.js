...
export async function checkEmojiMixSupport(emoji1, emoji2) {
    try {
        const emojiSupport = {
...
export async function animeRecognition(imageBuffer) {
    try {
...
export async function imageSearchGoogle(text) {
    try {
        ...
        const imagesResult = images.map(image => image.preview ? image : undefined).filter(image => image !== undefined);
        return imagesResult;
    }
    catch (err) {
